function randomNumberGenerator(multiplier: number) {
  return Math.round(Math.random() * multiplier);
}

function getRoundedModule(dividend: number, divisor: number) {
  return Math.round(dividend - Math.floor(dividend / divisor) * divisor);
}

export const generateCNPJ = ({ withPontuation }: GeneratorProps) => {
  const n = 9;
